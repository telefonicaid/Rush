...
if(config.casDir != ''){
  var casPath = path.resolve(__dirname, config.casDir);

  try{
    var casFiles = fs.readdirSync(casPath); // []
    ...
    logger.warning({ opType: 'GET CERTIFICATES', msg: err.code }, err.path);
  }
}
...
function createTask(simpleRequest, callback) {
  ...
  logger.debug({ opType: 'CREATE TASK', userID: simpleRequest.user, traceID: simpleRequest.traceID,
    msg: 'Arguments: '}, [ simpleRequest ]);
...
function doJob(task, callback) {
  ...
  logger.debug({ opType: 'DO JOB', userID: task.user, traceID: traceID, msg: 'Arguments: '}, [ task ]);
  ...
  if (!targetHost) {
    logger.warning({ opType: 'DO JOB', userID: task.user, traceID: traceID, msg: 'No target host' }, { id: task.id});
  } else {
    ...
    req = httpModule.request(options, function(rlyRes) {
      ...
      if (Math.floor(rlyRes.statusCode / 100) === 2) {
        ...
        getResponse(rlyRes, task, function(task, respObj) {
          ...
          logger.warning({ opType: 'DO JOB', userID: task.user, traceID: traceID, msg: 'Server Error (4xx, 5xx)' },
              { id: task.id, error: e });
    ...
    setTimeout(function(){
      if(!req.rush_connected) {
        logger.warning({ opType: 'DO JOB', userID: task.user,  traceID: traceID,
          msg: 'config.responseTimeout reached. Task cancelled' }, { id : task.id });
        req.abort();
      }
    ...
    req.on('error', function(e) {
      ...
      logger.warning({ opType: 'DO JOB', userID: task.user, traceID: traceID, msg: 'Request Error' },
          {id: task.id, error: e});
