...
function createTask(simpleRequest, callback) {
  ...
  if (! simpleRequest.headers[MG.HEAD_RELAYER_HOST]) {
    ...
    error.data = {
      exceptionId: 'SVC1000',
      exceptionText: 'Missing mandatory parameter: ' + MG.HEAD_RELAYER_HOST
    }
    ...
    //Check URLs
    var urlHeaders = [ MG.HEAD_RELAYER_HTTPCALLBACK, MG.HEAD_RELAYER_HTTPCALLBACK_ERROR, MG.HEAD_RELAYER_HOST];

    for (var i = 0; i < urlHeaders.length; i++) {
      var url = simpleRequest.headers[urlHeaders[i]];
      var urlError = urlErrors(url);
      ...
      if (urlError) {
        ...
        error.data = {
          exceptionId: 'SVC0002',
          exceptionText: 'Invalid parameter value: ' + urlHeaders[i],
          userMessage: urlError
        }
    ...
    if (retryStr) {
      ...
      if (! retrySplit.every(function(num) {
        ...
        error.data = {
          exceptionId: 'SVC0002',
          exceptionText: 'Invalid parameter value: ' + MG.HEAD_RELAYER_RETRY,
          userMessage: 'Invalid retry value: ' + retryStr
        }
    ...
    if (persistence) {
      if (persistence !== 'BODY' && persistence !== 'STATUS' &&
        ...
        error.data = {
          exceptionId: 'SVC0003',
          exceptionText: 'Invalid parameter value: ' + MG.HEAD_RELAYER_PERSISTENCE + '. Possible values are: BODY, ' +
              'STATUS, HEADER'
        }
...
function doJob(task, callback) {
  ...
  if (! targetHost) {
    ...
    req = httpModule.request(options, function(rlyRes) {
      ...
      if (Math.floor(rlyRes.statusCode / 100) === 2) {
        ...
        getResponse(rlyRes, task, function(task, respObj) {
          ...
          logger.warning('doJob - Server Error (4xx, 5xx)', e)
    ...
    req.on('error', function(e) {
      ...
      logger.warning('doJob - Request Error', e);
