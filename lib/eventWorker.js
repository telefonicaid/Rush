...
function createTask(simpleRequest, callback) {
  ...
  if (! simpleRequest.headers[MG.HEAD_RELAYER_HOST]) {
    ...
    var REGEX_HOSTNAME_PORT = new RegExp('^[a-zA-Z0-9\.\-]+(:([0-9]{1,5}))?$');   //Need to me improved
    ...
    if (!result) {
      ...
      if (port > MAX_PORT) {
        ...
        error.type = MG.INVALID_PARAMETER;
        error.parameter = MG.HEAD_RELAYER_HOST;
        error.userMessage = 'Port should be a number between 0 and ' + MAX_PORT;
...
function doJob(task, callback) {
  ...
  if (!targetHost) {
    ...
    var completeURL = protocol + '://' + targetHost + task.url;
