...
function validateHeaders(simpleRequest) {
  ...
  if (! simpleRequest.headers[MG.HEAD_RELAYER_HOST]) {
    ...
    if (retryStr) {
      ...
      var retryTimes = Number(retryStr);
      ...
      if (isNaN(retryTimes)) {
        ...
        error.type = MG.INVALID_PARAMETER;
        error.parameter = MG.HEAD_RELAYER_RETRY;
        error.userMessage = 'Invalid retry value: ' + retryStr + '. Retry value should be a natural number.';
...
function createTask(simpleRequest, callback) {
  ...
  logger.debug('New Task', { op: 'CREATE TASK', userID: simpleRequest.user, correlator: simpleRequest.traceID,
    transid: simpleRequest.id, arguments: [ simpleRequest ], force: simpleRequest.forceLog });
...
function delXrelayerHeaders(headers) {
  ...
  for (var h in headers) {
    if (headers.hasOwnProperty(h)) {
      var headerName = h.toLowerCase();
      if (headerName.indexOf('x-relayer') !== 0) {
        cleanHeaders[headerName] = headers[h];
      }
    }
  }
  return cleanHeaders;
...
function doJob(task, callback) {
  ...
  logger.debug('Do Job', { op: 'DO JOB', userID: task.user, correlator: traceID, transid: task.id,
    arguments: [ task ], force: task.forceLog });
  ...
  if (!targetHost) {
    logger.warning('No target host', { op: 'DO JOB', userID: task.user, correlator: traceID, force: task.forceLog,
      transid: task.id });
  } else {
    ...
    if (options.protocol === 'https:') {
      ...
      if (task.headers[MG.HEAD_RELAYER_SERVER_CERT]) {
        ...
        if (!config.trustAllServers && config.casDir) {
          options.ca = cas;
        }
      }
    ...
    //GET Request and DELETE request should not include content-length header since this type of petitions
    //does not include a body
    if (!options.headers['content-length'] && task.method !== 'GET' && task.method !== 'DELETE') {
      options.headers['content-length'] = Buffer.byteLength(task.body);
    }
      ...
      if (Math.floor(rlyRes.statusCode / 100) === 2) {
        ...
        getResponse(rlyRes, task, function(task, respObj) {
          ...
          logger.warning('Server Error (4xx, 5xx)', { op: 'DO JOB', userID: task.user, correlator: traceID,
            transid: task.id, error: e, force: task.forceLog });
    ...
    setTimeout(function(){
      if(!req.rushConnected) {
        logger.warning('config.responseTimeout reached. Task cancelled', { op: 'DO JOB', userID: task.user,
          correlator: traceID, transid : task.id, force: task.forceLog });
        req.abort();
      }
