...
function createTask(simpleRequest, callback) {
  ...
  logger.debug({ opType: 'CREATE TASK', traceID: simpleRequest.id, msg: 'Arguments: '}, [ simpleRequest ]);
...
function doJob(task, callback) {
  ...
  logger.debug({ opType: 'DO JOB', traceID: task.id, msg: 'Arguments: '}, [ task ]);
  ...
  if (!targetHost) {
    logger.warning({ opType: 'DO JOB', traceID: task.id, msg: 'No target host' });
  } else {
    ...
    req = httpModule.request(options, function(rlyRes) {
      ...
      if (Math.floor(rlyRes.statusCode / 100) === 2) {
        ...
        getResponse(rlyRes, task, function(task, respObj) {
          ...
          logger.warning({ opType: 'DO JOB', traceID: task.id, msg: 'Server Error (4xx, 5xx)' }, e);
    ...
    setTimeout(function(){
      if(!req.rush_connected) {
        logger.warning({ opType: 'DO JOB', traceID: task.id, msg: 'config.responseTimeout reached. Task cancelled' });
        req.abort();
      }
    ...
    req.on('error', function(e) {
      ...
      logger.warning({ opType: 'DO JOB', traceID: task.id, msg: 'Request Error' }, e);
