...
function createTask(simpleRequest, callback) {
  ...
  logger.debug('New Task', { op: 'CREATE TASK', userID: simpleRequest.user, unica_correlator: simpleRequest.traceID,
    transaction_id: simpleRequest.id, arguments: [ simpleRequest ] });
...
function doJob(task, callback) {
  ...
  logger.debug('Do Job', { op: 'DO JOB', userID: task.user, unica_correlator: traceID, transaction_id: task.id,
    arguments: [ task ]});
  ...
  if (!targetHost) {
    logger.warning('No target host', { op: 'DO JOB', userID: task.user, unica_correlator: traceID,
    ...
    req = httpModule.request(options, function(rlyRes) {
      ...
      if (Math.floor(rlyRes.statusCode / 100) === 2) {
        ...
        getResponse(rlyRes, task, function(task, respObj) {
          ...
          logger.warning('Server Error (4xx, 5xx)', { op: 'DO JOB', userID: task.user, unica_correlator: traceID,
            transaction_id: task.id, error: e });
    ...
    setTimeout(function(){
      if(!req.rush_connected) {
        logger.warning('config.responseTimeout reached. Task cancelled', { op: 'DO JOB', userID: task.user,
          unica_correlator: traceID, transaction_id : task.id });
        req.abort();
      }
    ...
    req.on('error', function(e) {
      ...
      logger.warning('Request Error', { op: 'DO JOB', userID: task.user, unica_correlator: traceID,
        transaction_id: task.id, error: e});
