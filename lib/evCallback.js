      ...
      function getHttpCallback(cbState, cbErrField) {
        return function onHttpCb(error, result) {
          if (error || result) {
            ...
            logger.info('Callback Ended', { userID: data.task.user, unica_correlator: traceID, op: 'HTTP CALLBACK',
              callback_state: st, transaction_id: data.task.id });
            emitter.emit(MG.EVENT_NEWSTATE, st);
          }
          if (error) {
            logger.warning('Callback Error', {  userID: data.task.user, unica_correlator: traceID, op: 'HTTP CALLBACK',
              transaction_id: data.task.id, error: error });
            var errev = {
              id: data.task.id,
              traceID: data.task.traceID,
              date: new Date()
...
function doHttpCallback(task, respObj, callbackHost, cbField, callback) {
  ...
  if (callbackHost) {
    ...
    var callbackReq = http.request(callbackOptions, function(callbackRes) {
      ...
      if (task.headers[MG.HEAD_RELAYER_PERSISTENCE]) {
        db.update(task.id, cbRes, traceID, task.user, function onUpdated(err) {
          if (err) {
            logger.warning('Redis Update Error', { userID: task.user, unica_correlator: traceID, op: 'HTTP CALLBACK',
              transaction_id: task.id, error: err });
          }
          if (callback) {
            callback(err, cbRes);
          }
    ...
    callbackReq.on('error', function onReqError(err) {
      //error in request
      if (err) {
        logger.warning('Request Error', { userID: task.user, unica_correlator: traceID, op: 'HTTP CALLBACK',
      ...
      if (task.headers[MG.HEAD_RELAYER_PERSISTENCE]) {
        db.update(task.id, cbSt, traceID, task.user, function onUpdated(err) {
          if (err) {
            logger.warning('Redis Update Error', { userID: task.user, unica_correlator: traceID,
              op: 'HTTP CALLBACK', transaction_id: task.id, error: err });
          }
          if (callback) {
            callback(cbSt, null);
          }
