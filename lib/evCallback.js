      ...
      function getHttpCallback(cbState, cbErrField) {
        return function onHttpCb(error, result) {
          if (error || result) {
            ...
            logger.info({ traceID: data.task.id, opType: 'HTTP CALLBACK', msg: 'Callback Ended' }, st);
            emitter.emit(MG.EVENT_NEWSTATE, st);
          }
          if (error) {
            logger.warning({ traceID: data.task.id, opType: 'HTTP CALLBACK',  msg: 'Callback Error'}, error);
            var errev = {
              id: data.task.id,
              topic: data.task.headers[MG.HEAD_RELAYER_TOPIC],
              date: new Date()
...
function doHttpCallback(task, respObj, callbackHost, cbField, callback) {
  ...
  if (callbackHost) {
    ...
    var callbackReq = http.request(callbackOptions, function(callbackRes) {
      ...
      if (task.headers[MG.HEAD_RELAYER_PERSISTENCE]) {
        db.update(task.id, cbRes, function onUpdated(err) {
          if (err) {
            logger.warning({ traceID: task.id, opType: 'HTTP CALLBACK', msg: 'Redis Update Error' }, err);
          }
          if (callback) {
            callback(err, cbRes);
          }
    ...
    callbackReq.on('error', function onReqError(err) {
      //error in request
      if (err) {
        logger.warning({ traceID: task.id, opType: 'HTTP CALLBACK', msg: 'Request Error' }, err);
      }
      var cbSt = {};
