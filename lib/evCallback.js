  ...
  if (callbackHost) {
    ...
    var callbackReq = http.request(callbackOptions, function(callbackRes) {
      ...
      if (task.headers[MG.HEAD_RELAYER_PERSISTENCE]) {
        db.update(task.id, cbRes, traceID, task.user, task.forceLog, function onUpdated(err) {
          if (err) {
            logger.warning('Redis Error', { userID: task.user, correlator: traceID, op: 'HTTP CALLBACK',
              transid: task.id, error: err, force: task.forceLog });
          }
          if (callback) {
            callback(err, cbRes);
          }
    ...
    callbackReq.on('error', function onReqError(err) {
      ...
      if (task.headers[MG.HEAD_RELAYER_PERSISTENCE]) {
        db.update(task.id, cbSt, traceID, task.user, task.forceLog, function onUpdated(err) {
          if (err) {
            logger.warning('Redis Error', { userID: task.user, correlator: traceID,
              op: 'HTTP CALLBACK', transid: task.id, error: err, force: task.forceLog });
          }
          if (callback) {
            callback(cbSt, null);
          }
