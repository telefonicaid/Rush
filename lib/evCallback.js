      ...
      function getHttpCallback(cbState, cbErrField) {
        return function onHttpCb(error, result) {
          if (error || result) {
            ...
            logger.info({ userID: data.task.user, traceID: traceID, opType: 'HTTP CALLBACK',
              msg: 'Callback Ended' }, st);
            emitter.emit(MG.EVENT_NEWSTATE, st);
          }
          if (error) {
            logger.warning({  userID: data.task.user, traceID: traceID, opType: 'HTTP CALLBACK',
              msg: 'Callback Error'}, {id: data.task.id, error: error });
            var errev = {
              id: data.task.id,
              traceID: data.task.traceID,
              date: new Date()
...
function doHttpCallback(task, respObj, callbackHost, cbField, callback) {
  ...
  if (callbackHost) {
    ...
    var callbackReq = http.request(callbackOptions, function(callbackRes) {
      ...
      if (task.headers[MG.HEAD_RELAYER_PERSISTENCE]) {
        db.update(task.id, cbRes, traceID, task.user, function onUpdated(err) {
          if (err) {
            logger.warning({ userID: task.user, traceID: traceID, opType: 'HTTP CALLBACK', msg: 'Redis Update Error' },
    ...
    callbackReq.on('error', function onReqError(err) {
      //error in request
      if (err) {
        logger.warning({ userID: task.user, traceID: traceID, opType: 'HTTP CALLBACK', msg: 'Request Error' },
            {id: task.id, error: err});
      }
      var cbSt = {};
