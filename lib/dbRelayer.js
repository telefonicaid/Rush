...
function update(key, obj, traceID, userID, forceLog, callback) {
  'use strict';
  ...
  logger.debug('Update Task', { component: logger.prefix + ' [redis]',userID: userID, op: 'HMSET',
    correlator: traceID, transid: key, arguments: [ config.keyPrefix + key, aux ], force: forceLog });
    ...
    db.expire(config.keyPrefix + key, configGlobal.expireTime, function(err) {
      if (err) {
        logger.error('Redis Error (expire)', { op: 'UPDATE KEY', correlator: traceID,
          userID: userID, transid: key, error: err, force: forceLog});
      }
...
function getData(key, traceID, userID, forceLog, callback) {
  'use strict';
  ...
  logger.debug('Get task data', { component: logger.prefix + ' [redis]', op: 'HGETALL', correlator: traceID,
    userID: userID, transid: key, force: forceLog, arguments: [ config.keyPrefix + key ] });
  ...
  db.hgetall(config.keyPrefix + key, function onHgetall(err, data) {
    if (err) {
      logger.warning('Redis Error (hgetall)', { op: 'GET TASK DATA', correlator: traceID,
        userID: userID, transid: key, force: forceLog, error: err });
      callback(err);
    }
    else {
      callback(null, data);
