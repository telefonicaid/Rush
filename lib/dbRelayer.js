...
function update(key, obj, userID, cllbk) {
  'use strict';
  ...
  logger.debug({ opType: 'UPDATE KEY', userID: userID, traceID: key, msg: 'Arguments: '}, [ obj ]);
  ...
  logger.debug({ component: logger.prefix + ' [redis]',userID: userID, opType: 'HMSET', traceID: key,
        msg: 'Arguments: '}, [ config.keyPrefix + key, o_aux ]);

  rcli.hmset(config.keyPrefix + key, o_aux, function onHmset(err, res) {

    logger.debug({ component: logger.prefix + ' [redis]', userID: userID, opType: 'EXPIRE', traceID: key,
    ...
    rcli.expire(config.keyPrefix + key, configGlobal.expireTime, function(err) {
      if (err) {
        logger.error({ opType: 'UPDATE KEY', traceID: key, userID: userID,  msg: 'Redis Error (expire)'},
...
function getData(key, userID, callback) {
  'use strict';
  ...
  logger.debug({ opType: 'GET TASK DATA', traceID: key, userID: userID, msg: 'Arguments: '}, [key]);
  ...
  rcli.hgetall(config.keyPrefix + key, function onHgetall(err, data) {
    if (err) {
      logger.warning({ opType: 'GET TASK DATA', traceID: key, userID: userID, msg: 'Redis Error (hgetall)' }, err);
      callback(err);
    }
    else {
      callback(null, data);
