...
function route(request, callback) {
  ...
  logger.debug({ opType: 'ROUTE REQUEST', userID: request.user, traceID: request.traceID,
    msg: 'Arguments: '}, [ request ]);
...
function getWorker(resp) {
  ...
  logger.debug({ opType: 'GET WORKER', userID: resp.task.user, traceID: resp.task.traceID,
    msg: 'Arguments: '}, [ resp ]);
