...
function route(request, callback) {
  ...
  logger.debug('Routing Req.', { op: 'ROUTE REQUEST', userID: request.user, unica_correlator: request.traceID,
    transaction_id: request.id, arguments: [ request ] });
...
function getWorker(resp) {
  ...
  logger.debug('Getting Worker', { op: 'GET WORKER', userID: resp.task.user, unica_correlator: resp.task.traceID,
    transaction_id: resp.task.id, arguments: [ resp ] });
