...
logger.info({ opType: 'START UP',  msg: 'RUSH_GEN_MONGO: '}, process.env.RUSH_GEN_MONGO);
...
function consume(idconsumer, start) {
  ...
  function processingConsumedTask(err, job) {
    var st;
    if (err) {
      logger.warning({ opType: 'CONSUME', msg: 'Error retrieving tasks'}, err);
      var errev = {
        ...
        if (job.queueId !== obsQueues.control) {
          ...
          do_job(job.task,
              function onJobEnd(dojoberr, jobresult) { //job results add
                if (dojoberr) {
                  logger.warning({ opType: 'CONSUME', userID: job.task.user, traceID: traceID, msg: 'Job Error' },
                      dojoberr);
                  //EMIT ERROR
                  var errev = {
                    id: job.task.id,
                  ...
                  logger.info({ opType: 'CONSUME', userID: job.task.user, traceID: traceID, msg: 'Job Ended' },
                      {id: job.task.id, result: jobresult});
                  //EMIT COMPLETED
                  st = {
                    id: job.task.id,
                    traceID: job.task.traceID,
                ...
                store.remProcessingQueue(idconsumer, function onRemoval(err) {
                  if (err) {
                    logger.warning({ opType: 'CONSUME', userID: job.task.user, traceID: traceID,
                          msg: 'Error removing processing queue' }, { id: job.task.id ,error: err });
                    //EMIT ERROR
                    var errev = {
                      idConsumer: idconsumer,
                      date: new Date(),
