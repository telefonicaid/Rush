        ...
        if (job.queueId !== obsQueues.control) {
          ...
          do_job(job.task,
              function onJobEnd(dojoberr, jobresult) { //job results add
                if (dojoberr) {
                  logger.warning({ opType: 'CONSUME', traceID: job.task.id, msg: 'Job Error' }, dojoberr)
                  //EMIT ERROR
                  var errev = {
                    id: job.task.id,
                    topic: job.task.headers[G.HEAD_RELAYER_TOPIC],
                  ...
                  logger.info({ opType: 'CONSUME', traceID: job.task.id, msg: 'Job Ended' },
                ...
                store.remProcessingQueue(idconsumer, function onRemoval(err) {
                  if (err) {
                    logger.warning({ opType: 'CONSUME', traceID: job.task.id, msg: 'Error removing processing queue' },
                        err);
                    //EMIT ERROR
                    var errev = {
                      idConsumer: idconsumer,
