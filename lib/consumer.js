...
async.parallel(evInitArray,
    function onSubscribed(err, results) {
      'use strict';
      if (err) {
        logger.error('Error subscribing event listener', { op: 'ADD-ONS START UP' }, err);
...
function consume(idconsumer, start) {
  ...
  function processingConsumedTask(err, job) {
    var st;
    if (err) {
      logger.warning('Error retrieving tasks', { op: 'CONSUME', error: err });
      var errev = {
        ...
        if (job.queueId !== obsQueues.control) {
          ...
          do_job(job.task,
              function onJobEnd(dojoberr, jobresult) { //job results add
                if (dojoberr) {
                  logger.warning('Job Error', { op: 'CONSUME', userID: job.task.user, unica_correlator: traceID,
                      error: dojoberr, transaction_id: job.task.id });
                  //EMIT ERROR
                  var errev = {
                    id: job.task.id,
                  ...
                  logger.info('Job Ended', { op: 'CONSUME', userID: job.task.user, unica_correlator: traceID,
                      result: jobresult, transaction_id: job.task.id });
                  //EMIT COMPLETED
                  st = {
                    id: job.task.id,
                    traceID: job.task.traceID,
                ...
                store.remProcessingQueue(idconsumer, function onRemoval(err) {
                  if (err) {
                    logger.warning('Error removing processing queue', { op: 'CONSUME', userID: job.task.user,
                      unica_correlator: traceID, id: job.task.id ,error: err });
                    //EMIT ERROR
                    var errev = {
                      idConsumer: idconsumer,
                      date: new Date(),
