...
log.setConfig(configGlobal.logger);
...
var obsQueues = serviceRouter.getQueues();
...
async.parallel(evInitArray,
    function onSubscribed(err/*, results*/) {
      'use strict';
      if (err) {
        logger.error('consumer could not be started', { op: 'CONSUMER START UP', error: 'Add-On Error: ' + err });
    ...
    if (err) {
      ...
      if (job && job.task) {
        ...
        if (job.queueId !== obsQueues.control) {
          var doJob = serviceRouter.getWorker(job);
          doJob(job.task,
              function onJobEnd(dojoberr, jobresult) { //job results add

                if (dojoberr) {
                  //Petition could not be completed due an error
                  logger.warning('Job Error', { op: 'CONSUME', userID: job.task.user, correlator: traceID,
                      error: dojoberr, transid: job.task.id, force: job.task.forceLog });
                ...
                } else if (jobresult) {   //Petition is completed only when jobresult is not null
                  //Petition completed
                  var logresult = {};
                  ...
                  if(logresult.body) {
                    if(typeof logresult.body === 'string' && configGlobal.logger.prefixBody > 0)  {
                      logresult.prefixBody = logresult.body.slice(0,configGlobal.logger.prefixBody);
                    }
                    delete logresult.body;
                  }

                  logger.info('Job Ended', { op: 'CONSUME', userID: job.task.user, correlator: traceID,
                      result: logresult, transid: job.task.id, force: job.task.forceLog });
                ...
                store.remProcessingQueue(idconsumer, function onRemoval(err) {
                  if (err) {
                    logger.warning('Error removing processing queue', { op: 'CONSUME', userID: job.task.user,
                      correlator: traceID, transid: job.task.id ,error: err, force: job.task.forceLog });
                    //EMIT ERROR
                    var errev = {
                      idConsumer: idconsumer,
                      date: new Date(),
                    ...
                    process.nextTick(function consumerClosure() {
...
function startConnections(done) {
  ...
  store.openConnections(function() {
    for (var i = 0; i < maxPoppers; i++) {
      consume(configGlobal.consumerID + i, true);
    }
