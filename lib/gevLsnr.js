...
function init(emitter, config) {
  ...
  logger.debug({ opType: 'INIT MONGO EV LISTENER', msg: 'Arguments: '}, [emitter, config]);
  ...
  return function(cbAsync) {
    ...
    function subscribeStateCol(callback) {
      client.collection(config.collection, function(err, c) {
        if (err) {
          logger.warning({ opType: 'INIT GEVENT LISTENER (STATE)', msg: 'Could not open the collection' }, err);
          if (callback) {
            callback(err);
          }
        } else {
            ...
            try {
              if (filterObj(data, config.filter)) {
                ...
                collection.insert(trimmed, function(err, docs) {
                  if (err) {
                    logger.warning({ userID: data.task.user, traceID: data.task.id, opType: 'GEVENT LISTENER (STATE)',
