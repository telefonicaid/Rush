  ...
  return function(callback) {
    ...
    emitter.on(MG.EVENT_NEWSTATE, function onNewEvent(data) {
      if (data.state === MG.STATE_ERROR || data.state === MG.STATE_COMPLETED ||
          data.state === MG.STATE_QUEUED || data.state === MG.STATE_PENDING) {

        var type, respObj;

        if (data.state === MG.STATE_QUEUED || data.state === MG.STATE_PROCESSING) {
          type = 'STATE';   //Notice the difference bt. 'STATE' and 'STATUS'
          respObj = { state : data.state };
        } else {
          type = data.state === MG.STATE_ERROR ? 'ERROR' : data.task.headers[MG.HEAD_RELAYER_PERSISTENCE];
...
function doPersistence(task, respObj, type, callback) {
  ...
  if (type === 'BODY' || type === 'STATUS' || type === 'HEADER' ||  //X-Relayer-Persistence valid values
      type === 'ERROR' || type === 'STATE') {                       //Internal values
    ...
    if (!type && callback) {
      ...
      if (callback) {
        ...
        logger.warning({ opType: 'PERSISTENCE', msg: errMsg, userID: task.user, traceID: task.traceID },
            { id: task.id });
        callback(errMsg);
      }
    }
