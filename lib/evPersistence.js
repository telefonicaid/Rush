  ...
  if (type === 'BODY' || type === 'STATUS' ||
    ...
    if (! type && callback) {
      ...
      if (callback) {
        ...
        logger.warning({ opType: 'PERSISTENCE', msg: 'Error', traceID: task.id }, errMsg);
        callback(errMsg);
      }
    }
  }
...
function setObject(task, respObj, type, callback) {
  ...
  db.update(task.id, setObj, function onUpdated(err) {
    ...
    if (err) {
      logger.warning({ opType: 'PERSISTENCE', msg: 'Redis Update Error', traceID: task.id }, err);
    } else {
      logger.info({ opType: 'PERSISTENCE', msg: 'Completed', traceID: task.id }, err);
    }
