  ...
  if (validValues.indexOf(type) >= 0) {
    ...
    if (!type && callback) {
      ...
      if (callback) {
        ...
        logger.warning(errMsg, { op: 'PERSISTENCE', userID: task.user, unica_correlator: task.traceID,
...
function setObject(task, respObj, type, callback) {
  ...
  db.update(task.id, setObj, traceID, task.user, function onUpdated(err) {
    ...
    if (err) {
      logger.warning('Redis Update Error', { op: 'PERSISTENCE', userID: task.user, unica_correlator: traceID,
        transaction_id: task.id, error: err });
    } else {
      logger.info('Persistence Completed', { op: 'PERSISTENCE', userID: task.user, unica_correlator: traceID,
        transaction_id: task.id });
    }
