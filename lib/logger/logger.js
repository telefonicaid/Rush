...
function newLogger() {
  ...
  //All components will use the same logger
  if (!logger && config) {
    ...
    logger.log = function (level, message, logObj) {
      ...
      for (var i = 0; i < logger.transports.length; i++) {
        ...
        if (globals.defaultLevels[level] <=  globals.defaultLevels[transport.level]) {
  ...
  } else if (!logger && !config) {
    throw new Error('Logger cannot be created without configuration');
  }
