...
function createDefaultLogger(config) {
  ...
  if (config) {
    ...
    if (logger.config.inspectDepth === undefined) {
      logger.config.inspectDepth = defaultConfig.inspectDepth;
    }

    if (logger.config.enableForceLog === undefined) {
      logger.config.enableForceLog = defaultConfig.enableForceLog;
    }
...
function newLogger() {
  ...
  moduleLogger.log = function(level, message, logObj) {
    ...
    myLogger.log(level, message, logObj);
  };
