  ...
  if (config) {
    ...
    logger.log = function (level, message, logObj) {
      ...
      for (var i = 0; i < this.transports.length; i++) {
        ...
        var transportLevel = transport.level || this.config.logLevel || defaultConfig.logLevel;

        if (globals.DEFAULTL_LEVELS[level] >= globals.DEFAULTL_LEVELS[transportLevel] || logObj[globals.FORCE]) {
