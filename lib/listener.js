    ...
    req.on('end', function onReqEnd() {
      ...
      assignRequest(req, data, function writeRes(result) {
        ...
        if (reqLog.headers[G.HEAD_RELAYER_TRACEID]) {
          logMsg.unica_correlator = reqLog.headers[G.HEAD_RELAYER_TRACEID];
        }

        if (result.data.id) {
          res.header('Location', '/response/' + result.data.id);
          logMsg.transaction_id = result.data.id;
        }
        //Send request
    ...
    req.on('end', function () {
      ...
      dbrelayer.getData(responseId, req.traceID, req.user, function (err, data) {
        ...
        if (err) {
          ...
          logger.warning('Error retrieving response', { op: 'RETRIEVE RESPONSE', transaction_id: responseId,
            unica_correlator: req.traceID, userID: req.user, error: err });
        } else {
          ...
          if (!data) {
            ...
            logger.warning('ID ' + responseId + ' does not exists',  { op: 'RETRIEVE RESPONSE',
              transaction_id: responseId, userID: req.user, unica_correlator: req.traceID });
          } else {
        ...
        logger.info('Retrieve Response received', { op: 'RETRIEVE RESPONSE', userID: req.user,
          unica_correlator: response.data.traceID, transaction_id: responseId, reqInfo: reqLog });
      });
    });
...
function assignRequest(request, data, callback) {
  ...
  function processTask(err, routeObj) {
    if (!err) {
      ...
      store.put(routeObj.service, routeObj.task, function onWrittenReq(error) {
        ...
        if (error) {
          logger.warning('Storage Error', { op: 'ASSIGN REQUEST', userID: request.user,
            unica_correlator: request.traceID, error: error });
      ...
      logger.warning('Petition Error', { op: 'ASSIGN REQUEST', userID: request.user, unica_correlator: request.traceID,
        error: err });
