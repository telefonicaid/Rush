...
function start(done){
  dbCluster.init(function(err){
    if(!err){
      ...
      logger.error('listener could not be started', { op: 'LISTENER START UP', error: err });
...
function startListening(done) {
  ...
  if (config.listener.enableSecure) {
    ...
    if (fs.existsSync(secureOptions.key) &&
      ...
      logger.warning('listener could not be started', { op: 'LISTENER START UP', additionalInfo: 'Certs not found',
