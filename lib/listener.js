...
async.parallel(evInitArray,
    function onSubscribed(err, results) {
      'use strict';
      if (err) {
        logger.error({ opType: 'ADD-ONS START UP', msg: 'Error subscribing event listener'}, err);
  ...
  function relayReq(req, res) {
    ...
    req.on('end', function onReqEnd() {
      ...
      assignRequest(req, data, function writeRes(result) {
        ...
        var logMsg = { opType: 'RELAY REQUEST', userID: req.user };

        if (reqLog.headers[G.HEAD_RELAYER_TRACEID]) {
          logMsg.traceID = reqLog.headers[G.HEAD_RELAYER_TRACEID];
        }
    ...
    req.on('end', function () {
      ...
      dbrelayer.getData(responseId, req.traceID, req.user, function (err, data) {
        ...
        if (err) {
          ...
          logger.warning( { opType: 'RETRIEVE RESPONSE', msg: 'Error retrieving response' }, err);
        } else {

          if (!data) {
            var error = { type: G.UNKNOWN_RESOURCE, resourceID: responseId };
            response = apiErrorsParser.parseError(error);
            logger.warning( { opType: 'RETRIEVE RESPONSE', msg: 'ID does not exists' }, responseId);
          } else {
        ...
        logger.info({ opType: 'RETRIEVE RESPONSE', userID: req.user, traceID: response.data.traceID }, reqLog);
      });
    });
  ...
  function processTask(err, routeObj) {
    if (!err) {
      ...
      store.put(routeObj.service, routeObj.task, function onWrittenReq(error) {
        var st;
        if (error) {
          logger.warning({ opType: 'ASSIGN REQUEST',  msg: 'Storage Error'}, error);
      ...
      logger.warning({ opType: 'ASSIGN REQUEST',  msg: 'Petition Error'}, err);
