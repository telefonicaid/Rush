          ...
          emitter.on(G.EVENT_NEWSTATE, function newEvent(data) {
            try {
              collection.insert(data, function(err, docs) {
                if (err) {
                  logger.warning({ traceID: data.task.id, opType: 'EVENT LISTENER (STATE)', msg: 'Insert Error' }, err);
                }
              });
            } catch (e) {
              logger.warning({ traceID: data.task.id, opType: 'EVENT LISTENER (STATE)', msg: 'Exception' }, e);
            }
          });
          if (callback) {
            callback(null);
        ...
        if (err) {
          ...
          emitter.on(G.EVENT_ERR, function newError(data) {
            try {
              collection.insert(data, function(err, docs) {
                if (err) {
                  logger.warning({ traceID: data.task.id, opType: 'EVENT LISTENER (ERRORS)', msg: 'Insert Error' },
                      err);
                }
              });
            } catch (e) {
              logger.warning({ traceID: data.task.id, opType: 'EVENT LISTENER (ERRORS)', msg: 'Exception' }, e);
            }
          });
          if (callback) {
            callback(null);
