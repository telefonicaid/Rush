          ...
          emitter.on(G.EVENT_NEWSTATE, function newEvent(data) {
            try {
              collection.insert(data, function(err, docs) {
                if (err) {
                  logger.warning({ userID: data.task.user, traceID: data.task.id, opType: 'EVENT LISTENER (STATE)',
        ...
        if (err) {
          ...
          emitter.on(G.EVENT_ERR, function newError(data) {
            try {
              collection.insert(data, function(err, docs) {
                if (err) {
                  logger.warning({ userID: data.task.user, traceID: data.task.id, opType: 'EVENT LISTENER (ERRORS)',
                        msg: 'Insert Error' },
                      err);
                }
              });
