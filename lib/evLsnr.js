          ...
          emitter.on(G.EVENT_NEWSTATE, function newEvent(data) {
            ...
            try {
              collection.insert(data, function(err, docs) {
                if (err) {
                  logger.warning('Insert Error', { userID: data.task.user, unica_correlator: traceID,
                    op: 'EVENT LISTENER (STATE)', transaction_id: data.task.id, error: err });
                }
              });
            } catch (e) {
              logger.warning('Exception', { userID: data.task.user, unica_correlator: traceID,
                op: 'EVENT LISTENER (STATE)', transaction_id: data.task.id, error: e });
            }
          });
          if (callback) {
            callback(null);
          ...
          emitter.on(G.EVENT_ERR, function newError(data) {
            ...
            try {
              collection.insert(data, function(err, docs) {
                if (err) {
                  logger.warning('Insert Error', { userID: data.task.user, unica_correlator: traceID,
                    op: 'EVENT LISTENER (ERRORS)', transaction_id: data.task.id, error: err });
                }
              });
            } catch (e) {
              logger.warning('Exception', { userID: data.task.user, unica_correlator: traceID,
                op: 'EVENT LISTENER (ERRORS)', transaction_id: data.task.id, error: e });
            }
          });
          if (callback) {
            callback(null);
