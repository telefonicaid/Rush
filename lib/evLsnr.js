              ...
              collection.insert(data, function(err, docs) {
                if (err) {
                  logger.warning('Insert Error', { userID: data.task.user, correlator: traceID,
                    op: 'EVENT LISTENER (STATE)', transid: data.task.id, error: err, force: data.task.forceLog });
                }
              });
            } catch (e) {
              logger.warning('Exception', { userID: data.task.user, correlator: traceID,
                op: 'EVENT LISTENER (STATE)', transid: data.task.id, error: e, force: data.task.forceLog });
            }
          });
          if (callback) {
            callback(null);
          ...
          emitter.on(G.EVENT_ERR, function newError(data) {
            ...
            try {
              collection.insert(data, function(err, docs) {
                if (err) {
                  logger.warning('Insert Error', { userID: data.task.user, correlator: traceID,
                    op: 'EVENT LISTENER (ERRORS)', transid: data.task.id, error: err, force: data.task.forceLog });
                }
              });
            } catch (e) {
              logger.warning('Exception', { userID: data.task.user, correlator: traceID,
                op: 'EVENT LISTENER (ERRORS)', transid: data.task.id, error: e, force: data.task.forceLog });
            }
          });
          if (callback) {
            callback(null);
