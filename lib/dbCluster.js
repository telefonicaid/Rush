...
var Connection = function(port, host, isOwn){
  ...
  cli.on('error', function(err){
    logger.error({ opType: 'REDIS CONNECTION', msg: 'Redis Error' }, err);
  });
...
=======
  ...
  self.monitor = setInterval(function(){
    if(!self.database.connected()){
      logger.warning({ opType: 'MONITOR', msg: 'disconnected' });
      self.emit('disconnected');
    } else {
      logger.info({ opType: 'MONITOR', msg: 'connected' });
      self.emit('connected');
...
=======
  ...
  if(config.minQuorum > sentsUp){
    logger.warning({ opType: 'CHECK SENTINELS',  msg: 'Impossible to reach an agreement between Sentinels'});
  } else {
