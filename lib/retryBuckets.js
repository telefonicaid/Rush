...
exports.initBucketTimers = function() {
          ...
          if (!errDel) {
            ...
            function insertOnMainQueue(errRoute, routeObj) {
              ...
              if (!errRoute) {
                ...
                logger.warn('Error repushing task from bucket', { op: 'RESPUSH BUCKET TASKS', userID: task.user,
  ...
  //Init bucket timers
  intervals = RETRY_BUCKETS.map(function(bucket, index) {
    var bucketName = BUCKET_PREFIX + String.fromCharCode(FIRST_CHARCODE + index);
    var intervalTime = bucket * 2 * 1000;
    return setInterval(repushBucketTasks.bind(this, bucketName), intervalTime);
  });
};
