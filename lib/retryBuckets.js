  ...
  function repushBucketTasks(bucketName, serviceQueue, cb) {
    ...
    db.eval(getTasksAndRepsuh, 2, bucketName, serviceQueue, function (errEval, data) {
      if (!errEval && data.length > 0) {
        ...
        logger.error('Error processing repush', { op: 'RESPUSH BUCKET TASKS', error: errEval });
        cb();
    ...
    if (services.hasOwnProperty(service)) {
      ...
      RETRY_BUCKETS.forEach(function(bucketTimeout, index) {
        ...
        async.whilst(
            ...
            function (callback) {
              repushBucketTasks(bucketName, serviceName, function(err) {
            ...
            function(err) {
              //Bucket has been stopped
              //This function is not called when an error arises getting elements from the bucket
            }
        );
